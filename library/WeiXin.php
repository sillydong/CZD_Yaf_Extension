...
class WeiXin{
	...
	public function responseMusicMessage($title,$description,$url,$hq_url=null){
		...
		$resultStr=sprintf($textTpl,$this->postObj->FromUserName,$this->postObj->ToUserName,$_SERVER['REQUEST_TIME'],
			self::REPLY_TYPE_MUSIC,$title,$description,$url,($hq_url==null?$url:$hq_url));
		if(!headers_sent()){
			header('Content-Type: application/xml; charset=utf-8');
		}
		echo $resultStr;
