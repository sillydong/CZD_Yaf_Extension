...
class SampleModel extends Object{
	...
	public static function authorize($passwd,$email,$nick,$phone){
		...
		if(!$result || !is_array($result)){
			...
			if((string)$result['passwd']===(string)md5($passwd)){
				return array('way'=>$way,'msg'=>'authorized','code'=>1,'user'=>new UserModel($result['id_user']));
			}
			else{
	...
	public static function inactivate($id_user){
		if(Validate::isUnsignedId($id_user)){
			$sql="update user set active=0 where id_user=".$id_user;
			Db::getInstance(true)->execute($sql);
		}
	}
